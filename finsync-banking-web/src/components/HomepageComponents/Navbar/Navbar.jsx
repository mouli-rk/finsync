  ...
  return (
    <nav className="p-4 md:px-[2rem] md:py-5 lg:px-8 lg:py-4 xl:px-20 xl:py-6 flex  items-center justify-between">
      <div className="logo w-1/2">
        <Link to="/" className="text-2xl md:text-3xl font-bold heading_font flex items-center">
          Fin
          <span className="inline-block mx-2 text-cyan-400">
            <FaSyncAlt />
          </span>
          Sync
        </Link>
      ...
      <div className="nav-container relative lg:w-full">
        ...
        <div
          ...
          } rounded-lg bg-cyan-400 bg-opacity-60 absolute top-10 right-0 transition-all duration-500 overflow-hidden lg:static lg:bg-transparent lg:w-full lg:flex justify-between`}
        >
          {/* Page Links */}
            ...
            {pageLinks.map((link, i) => {
              return (
                <NavLink key={i} to={link.url} className={activedLink}>
                  <span
                    className={`${styles.link_icon} text-2xl`}
                  >
                    {link.icon}
                  </span>
                  {link.title}
          ...
          <div className="p-3 gap-2 flex-col lg:p-0 flex lg:flex-row lg:gap-5">
            {authLinks.map((link, i) => {
              return (
                <NavLink key={i} to={link.url} className={activedLink}>
                  <span className={`${styles.link_icon} text-2xl`}>{link.icon}</span>
                  {link.title}
                </NavLink>
              );
            })}

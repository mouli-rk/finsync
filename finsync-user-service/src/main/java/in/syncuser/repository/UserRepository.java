...
public interface UserRepository extends JpaRepository<User, Long> {
	...
	@Query("""
			SELECT new in.syncuser.dto.UserApiDTO(u.id, u.username, u.password) FROM User u WHERE u.username =?1
			""")
	...
	@Query("""
			SELECT u.id AS id, u.code AS code, u.firstName AS firstName FROM User u
			WHERE (:code IS NULL OR u.code LIKE %:code%)
			""" )
	public List<UserDTO> findByCode(@Param("code") String code);

	@Query("""
			SELECT u.id AS id, u.firstName AS firstName, u.fullName AS fullName FROM User u 
	...
	@Query("""
			SELECT u.id AS id, u.code AS code, u.firstName AS firstName, u.lastName AS lastName, u.phoneNo AS phoneNo, 
